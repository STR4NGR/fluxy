<template>
  ...
  <div class="container mx-auto flex justify-between items-center">
    ...
    <div class="flex items-center pl-5 pt-1">
      ...
      <div class="flex items-center pl-5 pt-1">
        <!-- Кнопка влево -->
        <button @click="changeDayRange('left', -1)"
          class=" text-gray-300 px-2 py-2 hover:text-black transition-colors duration-300 flex items-center justify-center">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-5 h-5">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 15l-5-5 5-5" />
          </svg>
      ...
      <div class="flex items-center pl-5 pt-1">
        <button @click="changeDayRange('right', -1)"
          class=" text-gray-300 px-2 py-2 hover:text-black transition-colors duration-300 flex items-center justify-center">
          <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor" class="w-5 h-5">
            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 15l-5-5 5-5" />
          </svg>
    ...
    <div class="flex items-centers mr-6">
      ...
      <button v-if="isAdmin" @click="selectShop"
  ...
  <div class="container">
    <div class="bg-white p-6 rounded-lg -mt-8">
      <div class="overflow-x-auto mt-3" v-if="days.length">
        <table class="min-w-full table-auto border-collapse border border-gray-300 text-center text-xs"
          style="table-layout: fixed;">
          <thead>
            <tr class="bg-gray-200">
              <th class="border border-gray-300 py-2 text-gray-700 font-semibold bg-gray-200 z-10"
                style="width: 150px;">Продавец</th>
              ...
              <th v-for="(day, index) in filteredDays" :key="index" :class="getDayClass(day)"
                class="border border-gray-300 py-2" style="height: 100%; width: 110px; min-width: 50px;">
                {{ day.getDate() }}<br>({{ day.toLocaleDateString('ru-RU', { weekday: 'short' }) }})<br>{{
                calculateTotalHoursForDay(day) }}
              </th>
            </tr>
          </thead>
          <tbody>
            <tr v-for="(seller, sellerIndex) in sellers" :key="sellerIndex" class="border-t-4 border-gray-400">
              ...
              <td v-for="(day, index) in filteredDays" :key="index" class="border border-gray-300 p-0"
                style="height: 100%; width: 100px;">
                <div class="flex items-center justify-center w-full h-full">
                  <!-- Кнопка для каждой ячейки -->
                  <button class="custom-cellflex-grow h-full w-full py-1 px-1 bg-white border border-gray-300 text-sm"
                    :class="getCellClass(day, seller._id)" :style="getCellStyle(day)"
      ...
      <div class="bg-white p-6 rounded-lg">
        <div class="overflow-x-auto mt-3">
          <table class="min-w-full table-auto border-collapse border border-gray-300 text-center text-xs"
            ...
            <tbody>
              <tr v-for="(seller, sellerIndex) in sellers" :key="sellerIndex" class="border-t-4 border-gray-400">
                <td class="py-4 px-3 bg-gray-100 border-r border-gray-300 font-semibold text-center"
                  style="width: 150px;">
                  {{ seller.name }}
                </td>
                <td class="text-sm border border-gray-300" style="width: 150px;">
                ...
                <td class="text-sm border border-gray-300" style="width: 150px;">
                  <input type="number" v-model="seller.result.received5to7" min="0" @input="saveCalculation(seller)"
                    class="w-full text-center" />
                </td>
                <td class="text-sm border border-gray-300" style="width: 150px;">
                  <input type="number" v-model="seller.result.received15to17" min="0" @input="saveCalculation(seller)"
                    class="w-full text-center" />
                </td>
                <td class="text-sm border border-gray-300" style="width: 150px;">
                  <input type="number" v-model="seller.result.received25to27" min="0" @input="saveCalculation(seller)"
                    class="w-full text-center" />
                </td>
                <td class="text-sm border border-gray-300" style="width: 150px;">
                  <input type="number" v-model="seller.result.receivedYaMarket" min="0" @input="saveCalculation(seller)"
                    class="w-full text-center" />
                </td>
                <td class="text-sm border border-gray-300" style="width: 150px;">
                  <input type="number" v-model="seller.result.receivedOzonMarket" min="0"
                    @input="saveCalculation(seller)" class="w-full text-center" />
                </td>
                <td class="text-sm border border-gray-300" style="width: 150px;">
      ...
      <v-dialog v-model="isShiftTimeModalOpen" persistent max-width="550px" min-width="500px">
        <v-card>
            ...
            <v-row>
              <v-col cols="12" sm="6">
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center">
                    <button @click="decrementHour('start')"
                    ...
                    <button @click="incrementHour('start')"
                      class="text-gray-300 px-2 py-2 hover:text-black transition-colors duration-300">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 15l5-5-5-5" />
              ...
              <v-col cols="12" sm="6">
                <div class="flex items-center justify-between">
                  ...
                  <div class="flex items-center">
                    <button @click="decrementHour('end')"
                      class="text-gray-300 px-2 py-2 hover:text-black transition-colors duration-300">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M10 15l-5-5 5-5" />
                    ...
                    <button @click="incrementHour('end')"
                      class="text-gray-300 px-2 py-2 hover:text-black transition-colors duration-300">
                      <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke="currentColor"
                        class="w-5 h-5">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 15l5-5-5-5" />
          ...
          <v-card-actions>
            ...
            <v-btn color="primary" @click="saveShift('workday', shiftStart.hours, shiftEnd.hours)">Подтвердить
              время</v-btn>
          </v-card-actions>
        </v-card>
      </v-dialog>
